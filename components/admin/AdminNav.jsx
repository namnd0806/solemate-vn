...
export default function AdminNav() {
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto px-4 pb-5">
      <Link href="/admin/dashboard" className="group mt-4 flex items-center gap-3 rounded-[24px] border border-white/10 bg-white/[.06] p-4 transition hover:bg-white/[.09]">
        <span className="grid size-12 place-items-center rounded-2xl bg-primary text-lg font-black text-white shadow-[0_14px_30px_rgba(242,106,46,.26)]">S</span>
      ...
      <div className="mt-6 rounded-[24px] border border-white/10 bg-white/[.045] p-2">
        <p className="px-3 pb-2 pt-1 text-[10px] font-black uppercase tracking-[.22em] text-white/35">Menu quản trị</p>
          ...
          {NAV.map(([href, label, hint, icon]) => {
            ...
            return (
              <Link
                key={href}
                href={href}
                className={`group relative flex items-center gap-3 overflow-hidden rounded-2xl px-3 py-3 text-sm font-black transition duration-200 active:scale-[.985] ${active ? 'bg-primary text-white shadow-[0_16px_34px_rgba(242,106,46,.24)]' : 'text-white/62 hover:bg-white/[.075] hover:text-white'}`}
              >
                ...
                <span className={`h-2 w-2 rounded-full transition ${active ? 'bg-white' : 'bg-white/0 group-hover:bg-primary'}`} />
              </Link>
            )
          })}
        </nav>
      ...
      <div className="mt-auto rounded-[24px] border border-white/10 bg-[linear-gradient(145deg,rgba(255,255,255,.08),rgba(255,255,255,.025))] p-4">
